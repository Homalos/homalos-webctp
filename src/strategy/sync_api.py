...
class SyncStrategyApi:
    ...
    def __init__(
        ...
        instrument_info: dict[str, dict] | None = None,
    ):
        """
    ...
    def _query_instrument(
        ...
        try:
            ...
            # 构造合约查询请求
            from ..constants.constant import CommonConstant
            ...
            request = {
                ...
                Constant.ReqQryInstrument: {"InstrumentID": instrument_id},
            }

            logger.info(f"[合约查询] 请求构造完成: {request}")
            logger.debug(
                f"[合约查询] 请求字段详情 - MessageType: {Constant.ReqQryInstrument}, RequestID: 0, InstrumentID: {instrument_id}"
            )
            ...
            try:
                ...
                logger.error(
                    f"[合约查询] 提交请求超时（{timeout / 2}秒）: {instrument_id}"
                )
                raise TimeoutError(f"提交合约查询请求超时（{timeout / 2}秒）")
            except Exception as e:
                logger.error(
                    f"[合约查询] 提交请求失败: {instrument_id}, 错误: {e}",
                    exc_info=True,
                )
                raise
            ...
            if not self._event_manager.wait_event(
                f"instrument_query_{instrument_id}", timeout=timeout
            ):
                logger.error(f"[合约查询] 等待响应超时（{timeout}秒）: {instrument_id}")
                logger.warning(
                    "[合约查询] 可能的原因：1) SimNow环境不支持合约查询 2) 网络延迟 3) CTP服务器未响应"
                )
            ...
            if instrument_info:
                logger.info(
                    f"[合约查询] 查询成功: {instrument_id}, 乘数: {instrument_info.get('VolumeMultiple', 'N/A')}"
                )
            ...
            logger.error(
                f"[合约查询] 查询异常: {instrument_id}, 错误: {e}", exc_info=True
            )
            raise
        finally:
            # 清理查询事件
    ...
    def _on_market_data(self, response: dict) -> None:
        ...
        logger.debug(
            f"收到行情推送: {instrument_id}, 价格: {market_data.get('LastPrice')}"
        )
    ...
    def _handle_trade_report(self, response: dict) -> None:
        ...
        logger.debug(
            f"收到成交回报: {instrument_id}, 成交量: {trade_data.get('Volume', 0)}"
        )
        ...
        try:
            ...
            query_thread = threading.Thread(
                target=self._query_position_async, args=(instrument_id,), daemon=True
            )
            query_thread.start()
    ...
    def _on_trade_data(self, response: dict) -> None:
        ...
        if (
            "RtnOrder" in msg_type
            or "ErrRtnOrderInsert" in msg_type
            or "RspOrderInsert" in msg_type
        ):
        ...
        if "RspQryInstrument" in msg_type:
            ...
            logger.info(
                f"[TD回调-合约查询] 响应详情 - instrument_data存在: {instrument_data is not None}, IsLast: {is_last}, ErrorID: {error_id}, ErrorMsg: {error_msg}"
            )

            if error_id != 0:
                logger.error(
                    f"[TD回调-合约查询] 查询失败 - ErrorID: {error_id}, ErrorMsg: {error_msg}"
                )
            ...
            if instrument_data:
                ...
                logger.info(
                    f"[TD回调-合约查询] 收到合约数据: {instrument_id}, 乘数: {volume_multiple}"
                )
        ...
        logger.debug(
            f"收到持仓数据: {instrument_id}, 方向: {posi_direction}, 持仓: {position}"
        )
    ...
    def _query_position(self, instrument_id: str, timeout: float = 5.0) -> None:
        ...
        try:
            ...
            request = {
                ...
                "QryInvestorPosition": {"InstrumentID": instrument_id},
            }
            ...
            if not self._event_manager.wait_event(
                f"position_query_{instrument_id}", timeout=timeout
            ):
    ...
    def _subscribe_quote(self, instrument_id: str, timeout: float = 5.0) -> None:
        ...
        with self._subscription_lock:
            ...
            try:
                ...
                from ..constants.constant import MdConstant
                ...
                try:
                    anyio.from_thread.run(
                        md_client.call,
                        request,
                        token=self._event_loop_thread.anyio_token,
                    )
                except TimeoutError:
    ...
    def get_quote(self, instrument_id: str, timeout: float | None = None) -> Quote:
        """
        获取合约行情（同步阻塞）
    ...
    def get_position(
        self, instrument_id: str, timeout: float | None = None
    ) -> Position:
        """
        获取合约持仓（同步阻塞）
        ...
        has_position = (
            cached_position.pos_long > 0
            or cached_position.pos_short > 0
            or cached_position.pos_long_today > 0
            or cached_position.pos_short_today > 0
            or cached_position.pos_long_his > 0
            or cached_position.pos_short_his > 0
        )
        ...
        try:
            ...
            logger.info(
                f"获取到持仓数据: {instrument_id}, 多头: {position.pos_long}, 空头: {position.pos_short}"
            )
            return position
            ...
            # 查询超时，返回空持仓对象（不抛出异常）
            logger.warning(
                f"查询合约 {instrument_id} 持仓超时（{timeout}秒），返回空持仓"
            )
            return Position()
    ...
    def wait_quote_update(
        ...
        try:
            ...
            logger.warning(
                f"等待合约 {instrument_id} 行情更新超时（{timeout_str}），可能处于非交易时间段"
            )
            # 返回缓存中的最新行情（如果有）
            cached_quote = self._quote_cache.get(instrument_id)
            if cached_quote:
                logger.info(
                    f"返回缓存中的行情数据: {instrument_id}, 价格: {cached_quote.LastPrice}"
                )
                return cached_quote
            # 如果缓存中也没有，返回空行情对象
    ...
    def open_close(
        ...
        logger.info(
            f"提交订单: {instrument_id}, 动作: {action}, 数量: {volume}, 价格: {price}, 超时: {timeout}s"
        )
            ...
            if is_close_action:
                ...
                if need_distinguish:
                    ...
                    position = self.get_position(
                        instrument_id, timeout=self._config.position_timeout
                    )
                    ...
                    if len(orders_to_submit) > 1:
                        ...
                        for order_info in orders_to_submit:
                            ...
                            if not sub_result["success"]:
                                logger.error(
                                    f"{order_info['description']} 失败: {sub_result['error_msg']}"
                                )
                                return sub_result
                        ...
                        return {
                            "success": True,
                            "order_ref": ", ".join(
                                [r.get("order_ref", "") for r in results]
                            ),
                            "instrument_id": instrument_id,
                            "action": action,
                            "volume": volume,
                    ...
                    # 不需要拆分，确定使用平今还是平昨
                    close_today = (
                        bool(orders_to_submit[0]["close_today"])
                        if orders_to_submit
                        else False
                    )
                    logger.debug(
                        f"平仓操作: {orders_to_submit[0]['description'] if orders_to_submit else '无持仓'}"
                    )
    ...
    def run_strategy(
        self, strategy_func: Callable, *args, **kwargs
    ) -> threading.Thread:
        """
        在独立线程中运行策略
        ...
        with self._strategy_lock:
            active_count = sum(
                1 for t in self._running_strategies.values() if t.is_alive()
            )
        ...
        def strategy_wrapper():
            ...
            try:
                ...
                # 捕获策略异常，记录完整堆栈信息
                logger.error(f"策略 {strategy_name} 发生异常: {e}", exc_info=True)
    ...
    def stop(self, timeout: float | None = None) -> None:
        ...
        try:
            ...
            if running_strategies:
                ...
                # 为每个策略线程分配超时时间
                per_thread_timeout = (
                    timeout / len(running_strategies) if running_strategies else timeout
                )
...
# 导出公共接口
__all__ = ["Position", "Quote", "StrategyPlugin", "SyncStrategyApi"]
