...
1. P95 和 P99 之间差距小，说明延迟稳定性好
1. 所有场景下 P95 延迟均远低于目标值
...
1. **行情延迟测试**: 模拟单合约和多合约行情处理，记录处理延迟
1. **吞吐量测试**: 在固定时间内测量系统处理能力
1. **数据分析**: 计算统计指标并生成报告

### 测试限制

1. **模拟环境**: 使用 asyncio.sleep 模拟，不反映真实网络和 CTP API 延迟
1. **无 Redis**: 未连接 Redis，无法测试缓存效果
1. **无并发**: 单线程测试，未测试多客户端并发场景
1. **短时测试**: 测试时间较短（5-10秒），未测试长时间稳定性
...
1. **吞吐量达标**: 订单吞吐量超过目标，行情吞吐量受模拟环境限制
1. **稳定性良好**: 延迟波动小，P95 和 P99 接近，说明系统稳定
1. **可扩展性**: 高负载下性能仍然良好，有进一步优化空间
