...
class TestWaitQuoteUpdate:
    ...
    def test_wait_quote_update_auto_subscribe(self):
        ...
        with patch(
            ...
            side_effect=simulate_subscription_and_push,
        ):
    ...
    def test_wait_quote_update_multiple_waiters(self):
        ...
        assert (
            len(quotes) == 3
        ), f"所有 3 个等待线程都应该收到行情通知，实际收到: {len(quotes)}"
...
class TestPropertyWaitQuoteUpdate:
    ...
    def test_property_wait_quote_update_timestamp_advances(
        self, instrument_id: str, initial_price: float, updated_price: float
    ):
        """
        **Feature: sync-strategy-api, Property 3: 行情更新阻塞等待**
        ...
        **Validates: Requirements 1.3**
        ...
        # 验证 3：返回的行情合约代码正确
        assert (
            updated_quote.InstrumentID == instrument_id
        ...
        # 验证 5：返回的行情价格是更新后的价格
        assert (
            updated_quote.LastPrice == updated_price
        ), f"返回的价格 ({updated_quote.LastPrice}) 应该是更新后的价格 ({updated_price})"

        # 验证 6：返回的是新行情数据（不是初始行情）
        assert (
            updated_quote.UpdateTime == "09:30:01"
        ), "返回的行情时间应该是更新后的时间"
        assert updated_quote.UpdateMillisec == 200, "返回的行情毫秒应该是更新后的毫秒"
...
class TestPropertyQuoteNotificationBroadcast:
    ...
    def test_broadcast_basic(self):
        ...
        assert (
            len(quotes) == num_waiters
        ), f"所有 {num_waiters} 个等待线程都应该收到行情通知，但只有 {len(quotes)} 个收到"

        # 验证：没有线程超时
        assert len(errors) == 0, f"不应该有线程超时，但有 {len(errors)} 个线程超时"
    ...
    def test_property_all_waiters_receive_quote_update(
        self, num_waiters: int, price: float
    ):
        """
        **Feature: sync-strategy-api, Property 4: 行情通知广播**
        ...
        **Validates: Requirements 1.4**
        ...
        for i in range(num_waiters):
            thread = threading.Thread(
                target=wait_for_quote, name=f"Waiter-{i}", daemon=True
            )
            thread.start()
            wait_threads.append(thread)
        ...
        for i, quote in enumerate(quotes):
            ...
            assert (
                quote.InstrumentID == instrument_id
