...
from src.strategy.sync_api import Position, SyncStrategyApi, _EventLoopThread
...
class TestEventLoopExceptionHandling:
    ...
    def test_event_loop_exception_marks_service_unavailable(
        self, mock_td_client, mock_md_client
    ):
        """
        测试事件循环异常时标记服务不可用
    ...
    @patch("src.services.md_client.MdClient")
    @patch("src.services.td_client.TdClient")
    def test_event_loop_exception_logged(self, mock_td_client, mock_md_client):
        """
        测试事件循环异常被记录
        ...
        with patch("src.strategy.sync_api.logger") as mock_logger:
            ...
            assert any(
                "事件循环" in msg or "异常" in msg for msg in log_messages
            ), "日志应该包含事件循环异常信息"
...
class TestPropertyEventLoopExceptionHandling:
    ...
    @given(
        exception_type=st.sampled_from(
            [RuntimeError, ValueError, TypeError, ConnectionError, OSError, Exception]
        ),
    ...
    def test_property_event_loop_exception_handling(
        self, mock_td_client, mock_md_client, exception_type, error_message
    ):
        """
        Property 17: 异步事件循环异常处理
        ...
        with patch("src.strategy.sync_api.logger") as mock_logger:
            ...
            # 验证1：服务被标记为不可用
            assert (
                thread.is_service_available is False
            ), f"服务应该被标记为不可用（异常类型: {exception_type.__name__}）"

            # 验证2：初始化错误被保存
            assert thread._init_error is not None, "初始化错误应该被保存"
            assert isinstance(
                thread._init_error, exception_type
    ...
    @given(
        exception_type=st.sampled_from(
            [RuntimeError, ValueError, ConnectionError, OSError]
        )
    )
    ...
    def test_property_api_operations_fail_gracefully_when_service_unavailable(
        self, mock_td_client, mock_md_client, exception_type
    ):
        """
    ...
    @given(num_exceptions=st.integers(min_value=1, max_value=5))
    @settings(max_examples=20, deadline=None)
    @patch("src.services.md_client.MdClient")
    @patch("src.services.td_client.TdClient")
    def test_property_multiple_exceptions_handled_consistently(
        self, mock_td_client, mock_md_client, num_exceptions
    ):
        """
        ...
        for i in range(num_exceptions):
            ...
            with patch("src.strategy.sync_api.logger") as mock_logger:
                ...
                assert (
                    thread._init_error is not None
                ), f"第 {i+1} 次异常：错误应该被保存"

                assert mock_logger.error.called, f"第 {i+1} 次异常：应该记录错误日志"
