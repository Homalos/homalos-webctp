...
from src.constants import CommonConstant as Constant
from src.constants import MdConstant
...
class TestMarketDataCacheIntegration:
    ...
    @pytest.mark.asyncio
    async def test_market_data_push_to_redis_pubsub(self, md_client, cache_manager):
        """
        ...
        if len(received_messages) > 0:
            ...
            if cached_data is None:
                ...
                print("警告: fakeredis Pub/Sub 未能传递消息，这是测试环境限制")
                # 至少验证 md_client 调用了缓存方法
                assert cache_manager.is_available(), "CacheManager 应该可用"

    @pytest.mark.asyncio
    async def test_market_snapshot_cache_and_query(self, md_client, cache_manager):
        """
        测试行情快照缓存和查询
    ...
    @pytest.mark.asyncio
    async def test_market_data_ttl_expiration(self, md_client, cache_manager):
        """
        测试 TTL 过期
    ...
    @pytest.mark.asyncio
    async def test_redis_unavailable_degradation(self, md_client, metrics_collector):
        """
        测试 Redis 不可用时的降级
...
class TestMarketDataDistributionProperty:
        ...
        market_data=st.fixed_dictionaries(
            {
                ...
                "UpdateTime": st.text(alphabet="0123456789:", min_size=8, max_size=8).filter(
                    lambda x: x.count(":") == 2
                ),
            }
        )
    ...
    async def test_property_market_distribution_completeness(self, market_data):
        ...
        **Feature: performance-optimization-phase1, Property 3: 行情分发完整性**
        ...
        # 启动所有订阅者
        subscriber_tasks = [asyncio.create_task(subscriber(i)) for i in range(num_subscribers)]
        ...
        for i in range(1, num_subscribers):
            assert (
                received_messages[i][0] == first_message
