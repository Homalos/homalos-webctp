...
class TestFullStrategyIntegration:
    ...
    def test_complete_strategy_workflow(self, mock_event_loop_class):
        ...
        # ===== 2. 初始化 API =====
        api = SyncStrategyApi(
            user_id=TEST_USER_ID, password=TEST_PASSWORD, timeout=10.0
        )
...
class TestStrategyWithRealScenarios:
    ...
    def test_strategy_with_position_changes(self, mock_event_loop_class):
        ...
        api._position_cache.update_from_position_data(
            instrument_id, initial_position_data
        )
        ...
        api._position_cache.update_from_position_data(
            instrument_id, updated_position_data
        )
