...
from src.strategy.sync_api import Position, SyncStrategyApi
...
instrument_ids = st.text(
    alphabet=st.characters(whitelist_categories=("Lu", "Nd")), min_size=4, max_size=8
)
...
class TestGetPositionProperty:
    ...
    @given(instrument_id=instrument_ids, position_data=position_data_strategy)
    def test_property_get_position_returns_valid_object(self, instrument_id, position_data):
        ...
        **Validates: Requirements 2.1, 2.5**
        ...
        if math.isnan(expected_short_price):
            ...
            assert (
                position.open_price_short == expected_short_price
        ...
        if position.pos_long > 0:
            ...
            assert not math.isinf(position.open_price_long), "open_price_long 不应该是 Inf"
            assert (
        ...
        if position.pos_short > 0:
            ...
            assert not math.isinf(position.open_price_short), "open_price_short 不应该是 Inf"
            assert (
    ...
    @settings(max_examples=100)
    @given(instrument_id=instrument_ids)
    def test_property_cache_miss_triggers_query(self, instrument_id):
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        # 验证 1：应该触发了 CTP 查询
        assert query_called.is_set(), "缓存未命中时应该触发 CTP 查询，但没有调用 _query_position()"
    ...
    def test_cache_hit_does_not_trigger_query(self):
        ...
        # 验证：不应该触发查询
        assert not query_called.is_set(), "缓存命中时不应该触发 CTP 查询"
