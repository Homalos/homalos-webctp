...
class TestRunStrategy:
    ...
    def test_run_strategy_adds_to_registry(self):
        ...
        assert (
            strategies["test_strategy"] is thread
        ), "注册表中的线程对象应该与返回的一致"
    ...
    def test_run_strategy_thread_name(self):
        ...
        # 验证线程名称包含策略函数名
        assert (
            "my_custom_strategy" in thread.name
...
class TestPropertyStrategyThreadIndependence:
    ...
    def test_property_multiple_strategies_run_independently(
        self, num_strategies: int, sleep_time: float
    ):
        """
        **Feature: sync-strategy-api, Property 11: 策略线程独立运行**
        ...
        **Validates: Requirements 4.1, 4.2, 4.4**
        ...
        for i in range(num_strategies):
            ...
            assert strategy_name in strategies, f"策略 {strategy_name} 应该在注册表中"
        ...
        # 验证5：所有策略都执行完成
        assert (
            len(execution_records) == num_strategies
        ...
        # 验证7：策略在不同的线程中执行
        recorded_thread_ids = [
            record["thread_id"] for record in execution_records.values()
        ]
        assert len(recorded_thread_ids) == len(
            set(recorded_thread_ids)
        ), "每个策略应该在不同的线程中执行"
        ...
        for i in range(num_strategies):
            strategy_name = f"strategy_{i}"
            assert (
                strategy_name not in strategies
...
class TestPropertyStrategyExceptionIsolation:
    ...
    def test_property_exception_isolation(
        ...
        **Validates: Requirements 4.3, 7.4**
        ...
        for i in range(num_failing_strategies):
            key = f"failing_{i}"
            assert key in execution_records, f"失败策略 {i} 应该有执行记录"
            assert execution_records[key]["started"], f"失败策略 {i} 应该标记为已启动"

        # 验证5：正常策略都完成了（不受失败策略影响）
        for i in range(num_normal_strategies):
            key = f"normal_{i}"
            assert key in execution_records, f"正常策略 {i} 应该有执行记录"
            assert execution_records[key][
                "completed"
    ...
    @given(num_strategies=st.integers(min_value=3, max_value=5))
    @settings(max_examples=50, deadline=5000)
    def test_property_multiple_exceptions_isolation(self, num_strategies: int):
        """
        ...
        **Validates: Requirements 4.3, 7.4**
        ...
        # 验证1：所有策略都启动了
        assert (
            len(execution_status) == num_strategies
        ), f"应该有 {num_strategies} 个策略启动，实际: {len(execution_status)}"

        # 验证2：所有线程都已结束
        for strategy_id, thread in threads:
            assert not thread.is_alive(), f"策略 {strategy_id} 的线程应该已经结束"
    ...
    def test_property_exception_does_not_propagate(self):
        ...
        **Validates: Requirements 4.3, 7.4**
        ...
        # 验证：主线程没有收到异常
        assert not main_thread_exception[
            "occurred"
...
class TestPropertyStrategyRegistryMaintenance:
    ...
    def test_property_registry_maintenance(
        ...
        **Validates: Requirements 4.5**
        ...
        assert (
            len(strategies_after_clear) == num_strategies
        ...
        # 验证7：所有线程都已结束
        for strategy_name, thread in threads:
            assert not thread.is_alive(), f"策略 {strategy_name} 的线程应该已经结束"
        ...
        assert (
            len(final_strategies) == 0
        ), f"所有策略执行完成后，注册表应该为空，实际: {len(final_strategies)}"

        for strategy_name in strategy_names:
            assert (
                strategy_name not in final_strategies
            ), f"策略 {strategy_name} 执行完成后应该从注册表移除"
    ...
    def test_property_registry_cleanup_on_exception(
        self, num_normal_strategies: int, num_failing_strategies: int
    ):
        """
        ...
        **Validates: Requirements 4.5**
        ...
        strategies = api.get_running_strategies()
        assert (
            len(strategies) == total_strategies
    ...
    @given(num_strategies=st.integers(min_value=2, max_value=4))
    @settings(max_examples=50, deadline=5000)
    def test_property_registry_thread_consistency(self, num_strategies: int):
        """
        ...
        **Validates: Requirements 4.5**
        ...
        for strategy_name, returned_thread in returned_threads.items():
            ...
            assert registry_thread is not None, f"策略 {strategy_name} 应该在注册表中"

            assert (
                registry_thread is returned_thread
            ), f"注册表中的线程对象应该与返回的线程对象是同一个对象（策略: {strategy_name}）"

            # 验证线程 ID 一致
            assert (
                registry_thread.ident == returned_thread.ident
            ), f"注册表中的线程 ID 应该与返回的线程 ID 一致（策略: {strategy_name}）"

            # 验证线程名称一致
            assert (
                registry_thread.name == returned_thread.name
            ), f"注册表中的线程名称应该与返回的线程名称一致（策略: {strategy_name}）"

            # 验证线程状态一致
            assert (
                registry_thread.is_alive() == returned_thread.is_alive()
        ...
        for i in range(num_strategies):
            ...
            assert recorded_info is not None, f"策略 {i} 应该有线程信息记录"

            assert (
                recorded_info["thread_id"] == returned_thread.ident
            ), "策略执行期间的线程 ID 应该与返回的线程 ID 一致"

            assert (
                strategy_name in recorded_info["thread_name"]
            ), "策略执行期间的线程名称应该包含策略名称"

        # 验证3：注册表已清空
        time.sleep(0.1)
        final_registry = api.get_running_strategies()
        assert len(final_registry) == 0, "所有策略执行完成后，注册表应该为空"
    ...
    def test_property_registry_concurrent_access(self):
        ...
        **Validates: Requirements 4.5**
        ...
        assert (
            len(access_results) == expected_accesses
        ...
        for result in access_results:
            ...
            assert (
                0 <= registry_size <= num_strategies
