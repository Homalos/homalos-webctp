...
from src.strategy.sync_api import Quote, SyncStrategyApi
...
class TestPropertyAutoSubscribe:
    ...
    def test_property_auto_subscribe_on_first_get_quote(self, instrument_id: str):
        ...
        **Validates: Requirements 1.2**
        ...
        # 确保合约不在已订阅列表中
        assert (
            instrument_id not in api._subscribed_instruments
        ), "测试开始前合约不应该在已订阅列表中"
    ...
    def test_auto_subscribe_timeout(self):
        ...
        with patch(
            "asyncio.run_coroutine_threadsafe", side_effect=simulate_subscription_only
        ):
            # 调用 get_quote，应该超时
