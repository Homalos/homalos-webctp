...
async def strategy_manager(cache_manager_with_fake_redis):
    ...
    manager = StrategyManager(
        cache_manager=cache_manager_with_fake_redis, max_strategies=10
    )
    yield manager
...
class TestMultiStrategyParallel:
    ...
    async def test_multiple_strategies_run_concurrently(self, strategy_manager):
        ...
        for i in range(3):
            strategy_id = f"strategy_{i}"
            assert (
                len(call_counts[strategy_id]) > 0
            ), f"策略 {strategy_id} 应该收到行情数据"
    ...
    async def test_strategies_do_not_block_each_other(self, strategy_manager):
        ...
        await strategy_manager.register_strategy(
            "fast_strategy", fast_strategy, fast_config
        )
        await strategy_manager.register_strategy(
            "slow_strategy", slow_strategy, slow_config
        )
...
class TestMarketDataDistribution:
    ...
    async def test_market_data_distributed_to_all_subscribers(self, strategy_manager):
        ...
        for i in range(3):
            strategy_id = f"strategy_{i}"
            assert (
                len(received_data[strategy_id]) > 0
            ), f"策略 {strategy_id} 应该收到数据"
            assert received_data[strategy_id][0]["InstrumentID"] == "rb2505"
            assert received_data[strategy_id][0]["LastPrice"] == 3500.0

    @pytest.mark.asyncio
    async def test_strategies_receive_only_subscribed_instruments(
    ...
    async def test_multiple_market_data_broadcasts(self, strategy_manager):
        ...
        await strategy_manager.register_strategy(
            "counting_strategy", counting_strategy, config
        )
...
class TestStrategyCrashIsolation:
    ...
    async def test_strategy_error_does_not_stop_manager(self, strategy_manager):
        ...
        await strategy_manager.register_strategy(
            "crashing_strategy", crashing_strategy, crash_config
        )
        await strategy_manager.register_strategy(
            "stable_strategy", stable_strategy, stable_config
        )
        ...
        result = await strategy_manager.register_strategy(
            "new_strategy", new_strategy, new_config
        )
...
class TestResourceLimits:
    ...
    async def test_max_strategies_limit(self, cache_manager_with_fake_redis):
        ...
        # 创建限制为5个策略的管理器
        manager = StrategyManager(
            cache_manager=cache_manager_with_fake_redis, max_strategies=5
        )
        ...
        for i in range(5):
            ...
            result = await manager.register_strategy(
                f"strategy_{i}", dummy_strategy, config
            )
    ...
    async def test_unregister_frees_slot(self, cache_manager_with_fake_redis):
        """测试注销策略释放槽位"""
        manager = StrategyManager(
            cache_manager=cache_manager_with_fake_redis, max_strategies=3
        )
    ...
    async def test_strategy_without_subscriptions(self, strategy_manager):
        ...
        result = await strategy_manager.register_strategy(
            "no_sub_strategy", no_sub_strategy, config
        )
        assert result is True
...
class TestStrategyDynamicControl:
    ...
    async def test_start_stop_strategy_during_operation(self, strategy_manager):
        ...
        # 验证策略1没有收到新数据，策略2收到了
        assert (
            len(received_data["strategy_1"]) == count_before
        ), "停止的策略不应该收到新数据"
    ...
    async def test_restart_stopped_strategy(self, strategy_manager):
        ...
        # 注册并启动策略
        await strategy_manager.register_strategy(
            "restart_strategy", counting_strategy, config
        )
...
class TestComplexScenarios:
    ...
    async def test_high_frequency_broadcasts(self, strategy_manager):
        ...
        await strategy_manager.register_strategy(
            "high_freq_strategy", high_freq_strategy, config
        )
        ...
        # 验证策略收到了大部分行情（允许一些丢失）
        assert (
            received_count["strategy"] >= 90
        ), f"高频场景下应该收到大部分行情，实际收到: {received_count['strategy']}"
    ...
    async def test_strategy_with_multiple_instruments(self, strategy_manager):
        ...
        await strategy_manager.register_strategy(
            "multi_instrument_strategy", multi_instrument_strategy, config
        )
