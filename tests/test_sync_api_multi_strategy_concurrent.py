...
class TestMultiStrategyConcurrent:
    ...
    def test_multiple_strategies_different_instruments(self, mock_event_loop_class):
        ...
        completion_events = {
            instrument: threading.Event() for instrument in instruments
        }

        def create_strategy(
            instrument_id: str, expected_price: float, expected_position: int
        ):
            """创建策略函数"""
        ...
        for i, instrument_id in enumerate(instruments):
            ...
            ), f"{instrument_id} 的合约代码应该匹配"
            assert (
                quote.LastPrice == 3500.0 + i * 100
            ), f"{instrument_id} 的价格应该正确"
    ...
    def test_multiple_strategies_same_instrument(self, mock_event_loop_class):
        ...
        # 验证 1：所有策略都成功完成
        assert (
            len(strategy_results) == num_strategies
        ), f"应该有 {num_strategies} 个策略的结果"
    ...
    def test_mixed_success_and_failure_strategies(self, mock_event_loop_class):
        ...
        # 验证 3：异常策略失败但被捕获
        exception_results = [
            r for r in strategy_results.values() if r["type"] == "exception"
        ]
        ...
        # 验证 4：无效合约策略超时
        invalid_results = [
            r for r in strategy_results.values() if r["type"] == "invalid"
        ]
    ...
    def test_strategy_registry_management(self, mock_event_loop_class):
        ...
        with api._strategy_lock:
            ...
            assert (
                registry_size <= 1
            ), f"短时策略完成后，注册表应该只剩 0-1 个策略，实际有 {registry_size}"
        ...
        with api._strategy_lock:
            ...
            assert (
                registry_size == 0
            ), f"所有策略完成后，注册表应该为空，实际有 {registry_size}"
