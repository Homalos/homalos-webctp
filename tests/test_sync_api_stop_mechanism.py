...
class TestStopMechanism:
    ...
    def test_stop_waits_for_strategies(self):
        ...
        assert (
            len(api.get_running_strategies()) == num_strategies
        ...
        with lock:
            assert (
                len(strategy_completed) == num_strategies
            ), f"应该有 {num_strategies} 个策略完成"

        # 验证所有策略线程都已停止
        for thread in strategy_threads:
            assert not thread.is_alive(), f"策略线程 {thread.name} 应该已停止"
        ...
        assert (
            stop_duration >= expected_wait_time * 0.7
    ...
    def test_stop_cleans_up_resources(self):
        ...
        # 验证策略在注册表中
        assert len(api.get_running_strategies()) == 1, "应该有 1 个策略在运行"
        ...
        # 验证策略线程已停止
        assert not thread.is_alive(), "策略线程应该已停止"
