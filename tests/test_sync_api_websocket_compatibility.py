...
class TestWebSocketCompatibility:
    ...
    def test_independent_client_instances(self, mock_uvicorn_server, mock_event_loop_class):
        ...
        api = SyncStrategyApi(user_id=TEST_USER_ID, password=TEST_PASSWORD, timeout=10.0)
        ...
        assert (
            mock_ws_md_client.client_id != mock_sync_md_client.client_id
        ), "WebSocket 服务和 SyncStrategyApi 应该使用不同的 MD 客户端"
        assert (
            mock_ws_td_client.client_id != mock_sync_td_client.client_id
    ...
    def test_concurrent_operations_no_interference(self, mock_event_loop_class):
        ...
        assert sync_quote_updated.LastPrice == 70100.0, "SyncStrategyApi 的数据应该已更新"
    ...
    def test_resource_isolation(self, mock_event_loop_class):
        ...
        # 验证缓存对象不同（通过 id() 比较）
        assert id(api._quote_cache) != id(
            ws_quote_cache
        ), "SyncStrategyApi 和 WebSocket 服务应该使用不同的行情缓存对象"
        assert id(api._position_cache) != id(
            ws_position_cache
        ...
        # 验证事件管理器对象不同
        assert id(api._event_manager) != id(
            ws_event_manager
        ...
        # 验证策略注册表对象不同
        assert id(api._running_strategies) != id(
            ws_strategy_manager
...
class TestWebSocketAndSyncApiCoexistence:
    ...
    def test_no_shared_state_between_services(self, mock_event_loop_class):
        ...
        assert (
            "au2506" not in ws_state["subscribed_instruments"]
        ), "WebSocket 订阅列表不应该包含 SyncStrategyApi 的合约"
        assert ws_state["last_update_time"] == "09:30:00", "WebSocket 更新时间应该不变"
