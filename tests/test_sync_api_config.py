...
class TestSyncApiConfig:
    ...
    def test_config_file_not_found_uses_defaults(self, caplog):
        ...
        assert (
            api._config.connect_timeout == default_config.connect_timeout
        ), "应该使用默认的 ConnectTimeout"
        assert (
            api._config.max_strategies == default_config.max_strategies
        ), "应该使用默认的 MaxStrategies"
        assert (
            api._config.quote_timeout == default_config.quote_timeout
        ), "应该使用默认的 QuoteTimeout"
        assert (
            api._config.position_timeout == default_config.position_timeout
        ), "应该使用默认的 PositionTimeout"
        assert (
            api._config.order_timeout == default_config.order_timeout
        ), "应该使用默认的 OrderTimeout"
        assert (
            api._config.quote_update_timeout == default_config.quote_update_timeout
        ), "应该使用默认的 QuoteUpdateTimeout"
        assert api._config.stop_timeout == default_config.stop_timeout, "应该使用默认的 StopTimeout"
    ...
    def test_config_without_sync_api_section(self):
        ...
        try:
            ...
            with patch("src.strategy.sync_api._EventLoopThread") as mock_event_loop:
                ...
                # 初始化 API
                api = SyncStrategyApi(
                    user_id="test_user", password="test_password", config_path=path
                )
    ...
    def test_partial_sync_api_config(self):
        ...
        try:
            ...
            with patch("src.strategy.sync_api._EventLoopThread") as mock_event_loop:
                ...
                # 验证已配置的参数
                assert api._config.connect_timeout == 50.0, "已配置的 ConnectTimeout 应该使用配置值"
                assert api._config.max_strategies == 15, "已配置的 MaxStrategies 应该使用配置值"
    ...
    def test_config_values_are_correct_types(self, temp_config_file):
        ...
        # 验证类型
        assert isinstance(api._config.connect_timeout, float), "ConnectTimeout 应该是 float 类型"
        assert isinstance(api._config.max_strategies, int), "MaxStrategies 应该是 int 类型"
        assert isinstance(api._config.quote_timeout, float), "QuoteTimeout 应该是 float 类型"
        assert isinstance(api._config.position_timeout, float), "PositionTimeout 应该是 float 类型"
        assert isinstance(api._config.order_timeout, float), "OrderTimeout 应该是 float 类型"
        assert isinstance(
            api._config.quote_update_timeout, float
        ), "QuoteUpdateTimeout 应该是 float 类型"
        assert isinstance(api._config.stop_timeout, float), "StopTimeout 应该是 float 类型"
