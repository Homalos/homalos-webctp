...
position_strategy = st.fixed_dictionaries(
    {
        ...
        "PositionCost": st.floats(
            min_value=0, max_value=1000000000, allow_nan=False, allow_infinity=False
        ),
    }
)
...
async def test_cache_account_data(td_client_with_cache, mock_cache_manager):
    ...
    account_data = {
        ...
        Constant.TradingAccount: {"Balance": 100000.0, "Available": 50000.0},
    }
...
@pytest.mark.asyncio
async def test_cache_degradation_when_unavailable(
    td_client_with_cache, mock_cache_manager
):
    """测试 Redis 不可用时的降级处理"""
    # 模拟 Redis 不可用
    ...
    position_data = {
        Constant.MessageType: Constant.OnRspQryInvestorPosition,
        Constant.InvestorPosition: {"InstrumentID": "cu2501", "Position": 10},
    }
...
@pytest.mark.asyncio
async def test_query_position_cached_single_instrument(
    td_client_with_cache, mock_cache_manager
):
    """测试查询单个合约的持仓缓存"""
    ...
    position_data = {"InstrumentID": "cu2501", "Position": 10, "Direction": "0"}
...
@pytest.mark.asyncio
async def test_query_position_cached_all_instruments(
    td_client_with_cache, mock_cache_manager
):
    """测试查询所有合约的持仓缓存"""
    from src.utils.serialization import get_msgpack_serializer
    ...
    # 模拟 Redis 返回所有持仓数据
    mock_cache_manager.hgetall = AsyncMock(
        return_value={
...
@pytest.mark.asyncio
async def test_query_position_cached_not_found(
    td_client_with_cache, mock_cache_manager
):
    """测试查询不存在的持仓缓存"""
    # 模拟 Redis 返回 None
...
async def test_query_account_cached(td_client_with_cache, mock_cache_manager):
    ...
    serializer = get_msgpack_serializer()
    account_data = {"Balance": 100000.0, "Available": 50000.0}
...
async def test_query_orders_cached(td_client_with_cache, mock_cache_manager):
    ...
    member_1 = f"000001:{serialized_1.hex()}".encode()
    member_2 = f"000002:{serialized_2.hex()}".encode()
...
@pytest.mark.asyncio
async def test_refresh_cache_redis_unavailable(
    td_client_with_cache, mock_cache_manager
):
    """测试 Redis 不可用时刷新缓存"""
    # 模拟 Redis 不可用
...
async def test_property_position_cache_consistency(position_data):
    ...
    **Feature: performance-optimization-phase1, Property 1: 缓存一致性**
    ...
    with patch("src.services.td_client.CTPTdClient"):
        ...
        ), f"缓存数据不应为空: instrument_id={instrument_id}"

        # 验证关键字段
        assert (
            cached_data["InstrumentID"] == position_data["InstrumentID"]
        ), f"InstrumentID 不一致: 原始={position_data['InstrumentID']}, 缓存={cached_data['InstrumentID']}"
        assert (
            cached_data["Position"] == position_data["Position"]
        ), f"Position 不一致: 原始={position_data['Position']}, 缓存={cached_data['Position']}"
        assert (
            cached_data["Direction"] == position_data["Direction"]
...
async def test_property_account_cache_consistency(account_data):
    ...
    **Feature: performance-optimization-phase1, Property 1: 缓存一致性**
    ...
    with patch("src.services.td_client.CTPTdClient"):
        ...
        # 验证关键字段
        assert (
            cached_data["Balance"] == account_data["Balance"]
        ), f"Balance 不一致: 原始={account_data['Balance']}, 缓存={cached_data['Balance']}"
        assert (
            cached_data["Available"] == account_data["Available"]
        ...
        for key, value in account_data.items():
            ...
            if isinstance(value, float):
                assert (
                    abs(cached_data[key] - value) < 1e-6
                ), f"字段 {key} 不一致: 原始={value}, 缓存={cached_data[key]}"
            else:
                assert (
                    cached_data[key] == value
