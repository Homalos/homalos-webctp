...
from src.strategy.sync_api import _PositionCache, _QuoteCache
...
class TestAsyncCallbackThreadSafeNotification:
    ...
    def test_property_async_callback_thread_safe_notification(self, market_data_updates):
        ...
        **Validates: Requirements 8.4**
        ...
        for instrument_id in instruments:
            for waiter_id in range(num_waiters_per_instrument):
                thread = threading.Thread(
                    target=waiter_thread, args=(instrument_id, waiter_id), daemon=True
                )
        ...
        # 验证：不应该有任何异常
        assert len(exceptions) == 0, f"线程执行中出现异常: {exceptions}"
        ...
        # 至少应该收到一些更新
        assert actual_updates > 0, "没有线程收到更新，期望至少收到一些更新"

        # 验证：接收到的数据应该是有效的
        for (instrument_id, waiter_id), update_data in received_updates.items():
            # 验证合约代码正确
            assert (
                update_data["instrument_id"] == instrument_id
            ...
            # 不强制要求匹配特定的更新
            assert update_data["instrument_id"] != "", "合约代码不应该为空"
    ...
    def test_market_data_callback_notifies_multiple_waiters(self):
        ...
        # 验证：所有等待线程都应该收到通知
        assert (
            len(received_quotes) == num_waiters
    ...
    def test_position_callback_updates_cache_thread_safely(self):
        ...
        # 验证：所有读取操作都应该成功
        assert (
            len(read_results) == num_readers * num_updates
        ...
        for result in read_results:
            # 持仓数量应该在合理范围内
            assert (
                10 <= result["pos_long"] <= 10 + num_updates
            ), f"持仓数量超出预期范围: {result['pos_long']}"

            # 今仓应该在合理范围内
            assert (
                5 <= result["pos_long_today"] <= 5 + num_updates
            ), f"今仓数量超出预期范围: {result['pos_long_today']}"

            # 开仓均价应该在合理范围内
            assert (
                3500.0 <= result["open_price_long"] <= 3500.0 + num_updates
    ...
    def test_concurrent_callbacks_and_waiters(self):
        ...
        for instrument_id in instruments:
            for waiter_id in range(num_waiters_per_instrument):
                thread = threading.Thread(
                    target=waiter, args=(instrument_id, waiter_id), daemon=True
                )
                waiter_threads.append(thread)
                thread.start()
        ...
        assert (
            len(received_updates) == expected_count
        ...
        for (
            ...
            waiter_id,
        ), update_data in received_updates.items():
            actual_instrument_id = update_data["instrument_id"]
            assert (
                actual_instrument_id == expected_instrument_id
            ...
            assert (
                update_data["last_price"] == expected_price_base
    ...
    def test_multiple_callbacks_same_instrument(self):
        ...
        # 验证：应该接收到所有更新
        assert (
            len(received_prices) == num_updates
