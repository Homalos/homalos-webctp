...
class TestPropertyGracefulStop:
    ...
    def test_property_graceful_stop_waits_for_threads(
        self, num_strategies: int, strategy_duration: float
    ):
        """
        **Feature: sync-strategy-api, Property 19: 优雅停止等待线程**
        ...
        **Validates: Requirements 9.2**
        ...
        for i in range(num_strategies):
            thread = api.run_strategy(test_strategy, strategy_id=i, duration=strategy_duration)
        ...
        with state_lock:
            assert (
                len(strategy_states["started"]) == num_strategies
        ...
        with state_lock:
            assert (
                len(strategy_states["completed"]) == num_strategies
            ), f"期望 {num_strategies} 个策略完成，实际完成 {len(strategy_states['completed'])} 个"

            assert (
                len(strategy_states["interrupted"]) == 0
        ...
        assert (
            total_duration >= strategy_duration * 0.8
        ), f"总时间 {total_duration:.3f}s 小于策略运行时间 {strategy_duration:.3f}s"

        # 验证 3: 所有策略线程都已停止
        for thread in strategy_threads:
            assert not thread.is_alive(), f"策略线程 {thread.name} 在 stop() 返回后仍在运行"

        # 验证 4: 策略注册表已清空
        assert len(api.get_running_strategies()) == 0, "stop() 后策略注册表应该为空"
...
class TestGracefulStopEdgeCases:
    ...
    def test_stop_with_already_completed_strategies(self):
        ...
        assert stop_duration < 0.5, f"stop() 等待时间 {stop_duration:.3f}s 过长，策略已完成"
