...
class TestTradeReportTriggersPositionUpdate:
    ...
    def test_trade_report_triggers_position_query(self):
        ...
        assert (
            queried_instrument[0] == "rb2505"
        ), f"应该查询 rb2505，实际查询: {queried_instrument[0]}"
    ...
    def test_multiple_trade_reports_trigger_multiple_queries(self):
        ...
        for inst_id in instruments:
            trade_response = {
                "msg_type": Constant.OnRtnTrade,
                Constant.Trade: {"InstrumentID": inst_id, "Volume": 1, "Price": 3500.0},
            }
            api._on_trade_data(trade_response)
        ...
        # 验证：应该触发了多次查询
        assert (
            len(queried_instruments) == 3
        ), f"应该查询了 3 个合约，实际: {len(queried_instruments)}"
        assert set(queried_instruments) == set(
            instruments
