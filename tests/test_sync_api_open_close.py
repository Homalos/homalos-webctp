...
class TestOrderSubmissionMapping:
    ...
    def test_property_order_submission_correct_mapping(
        self, instrument_id: str, action: str, volume: int, price: float
    ):
        """
        **Feature: sync-strategy-api, Property 8: 订单提交正确映射**
        ...
        **Validates: Requirements 3.1, 3.2-3.5**
        ...
        assert offset_flag in [
            "0",
            "1",
        ], f"CombOffsetFlag 必须是 '0' 或 '1'，实际: {offset_flag}"

    @settings(max_examples=50)
    @given(action=st.sampled_from(["kaiduo", "kaikong", "pingduo", "pingkong"]))
    ...
    def test_property_invalid_action_raises_error(self, invalid_action: str):
        ...
        assert (
            invalid_action in error_msg or "不支持" in error_msg or "无效" in error_msg
    ...
    def test_all_actions_have_unique_mappings(self):
        ...
        for action in actions:
            ...
            # 验证这个映射组合是唯一的
            assert (
                mapping_tuple not in mappings.values()
...
class TestBlockingBehavior:
    ...
    def test_property_blocking_vs_nonblocking_behavior(self):
        ...
        **Validates: Requirements 3.6, 3.7**
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run:
            ...
            # 验证：包含订单号
            assert "order_ref" in result, f"返回结果应该包含 order_ref，实际: {result}"
    ...
    def test_property_blocking_timeout_behavior(self):
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run:
            ...
            assert (
                0.3 <= elapsed_time <= 0.7
            ), f"超时应该在 0.5 秒左右发生，实际: {elapsed_time:.2f} 秒"

            # 验证：异常消息应该包含超时信息
            assert (
                "超时" in str(exc_info.value)
    ...
    def test_block_true_waits_for_error_response(self):
        ...
        def create_delayed_error_future():
            ...
            def set_error_after_delay():
                ...
                future.set_result(
                    {
                        "RspInfo": {"ErrorID": 1, "ErrorMsg": "资金不足"},
                        "InputOrder": {"OrderRef": "123456"},
                    }
                )
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run:
            ...
            # 验证：返回结果应该标记为失败
            assert result["success"] is False, f"错误响应应该标记为失败，实际: {result}"

            # 验证：返回结果应该包含错误信息
            assert (
                "error_id" in result and result["error_id"] == 1
            ), f"返回结果应该包含错误代码，实际: {result}"

            assert (
                "error_msg" in result and "资金不足" in result["error_msg"]
...
class TestOrderFailureMarking:
    ...
    def test_property_order_failure_marking(
        ...
        **Validates: Requirements 3.8, 7.3**
        ...
        # 创建错误响应
        error_response = {
            "RspInfo": {"ErrorID": error_id, "ErrorMsg": error_msg},
            "InputOrder": {"OrderRef": "123456"},
        }
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run:
            ...
            assert result["price"] == price, f"返回结果应该包含 price，实际: {result}"
    ...
    def test_property_invalid_action_failure_marking(
        self, instrument_id: str, invalid_action: str, volume: int, price: float
    ):
        """
        ...
        **Validates: Requirements 3.8, 7.3**
        ...
        # 验证：返回结果应该包含错误信息
        assert "error_id" in result, f"返回结果应该包含 error_id 字段，实际: {result}"

        assert "error_msg" in result, f"返回结果应该包含 error_msg 字段，实际: {result}"

        # 验证：错误消息应该提示 action 无效
        error_msg = result["error_msg"]
        assert (
            invalid_action in error_msg or "无效" in error_msg or "不支持" in error_msg
        ), f"错误消息应该包含无效的 action 或提示信息，实际: {error_msg}"
    ...
    def test_property_system_error_failure_marking(self):
        ...
        **Validates: Requirements 7.3**
        ...
        error_msg = str(exc_info.value)
        assert (
            "事件循环" in error_msg or "connect" in error_msg
    ...
    def test_failure_result_structure(
        ...
        **Validates: Requirements 3.8, 7.3**
        ...
        # 创建错误响应
        error_response = {
            "RspInfo": {"ErrorID": 42, "ErrorMsg": "测试错误"},
            "InputOrder": {"OrderRef": "123456"},
        }
    ...
    def test_exception_during_submission_failure_marking(self):
        ...
        **Validates: Requirements 7.3**
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run:
            ...
            # 验证：错误消息应该包含异常信息
            assert (
                "模拟的提交异常" in result["error_msg"]
                or "提交订单失败" in result["error_msg"]
    ...
    def test_success_vs_failure_result_consistency(self):
        ...
        **Validates: Requirements 3.8**
        ...
        for field in common_fields:
            assert field in success_result, f"成功结果应该包含字段 '{field}'"
            assert field in failure_result, f"失败结果应该包含字段 '{field}'"
