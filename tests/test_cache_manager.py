...
class TestCacheManagerPubSub:
    ...
    async def test_subscribe_and_publish(
        self, cache_manager_with_fake_redis, serializer
    ):
...
class TestCacheManagerSortedSet:
    ...
    async def test_zrange_with_scores(self, cache_manager_with_fake_redis):
        ...
        # 获取成员和分数
        members_with_scores = await cache.zrange(
            "test:zset:scores", 0, -1, withscores=True
        )
        ...
        # 检查是否包含 member1
        members = [
            item[0] if isinstance(item, tuple) else item for item in members_with_scores
        ]
...
class TestCacheManagerConnectionFailure:
    ...
    async def test_operation_timeout(self, cache_manager_with_fake_redis):
        ...
        # 模拟超时
        with patch.object(cache._redis, "get", side_effect=TimeoutError()):
...
async def test_property_degradation_transparency(data):
    ...
    **Feature: performance-optimization-phase1, Property 2: 降级透明性**
    ...
    for key, value in data:
        ...
        assert (
            result is False
        ), "Delete 操作在 Redis 不可用时应返回 False，而不是抛出异常"
    ...
    **Feature: performance-optimization-phase1, Property 1: 缓存一致性**
    ...
    try:
        ...
        for key, value in data.items():
            ...
            assert (
                deserialized == value
            ), f"缓存一致性验证失败: 原始={value}, 读取={deserialized}"
    finally:
        await cache.close()
