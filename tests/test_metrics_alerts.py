        ...
        with patch("psutil.cpu_percent", mock_cpu_percent):
            with patch("psutil.virtual_memory") as mock_memory:
                mock_memory.return_value = MagicMock(
                    percent=50.0, used=1024 * 1024 * 1024
                )
            ...
            with patch("psutil.virtual_memory", return_value=mock_memory):
                with patch("psutil.net_io_counters") as mock_net:
                    ...
                    with patch("src.utils.metrics.logger") as mock_logger:
                        ...
                        assert any(
                            "延迟告警" in w for w in all_warnings
                        ), "延迟告警应该被触发"
                        assert any(
                            "Redis 命中率告警" in w for w in all_warnings
                        ), "Redis 命中率告警应该被触发"
                        assert any(
                            "CPU 使用率告警" in w for w in all_warnings
                        ), "CPU 使用率告警应该被触发"
                        assert any(
                            "内存使用率告警" in w for w in all_warnings
                        ), "内存使用率告警应该被触发"
