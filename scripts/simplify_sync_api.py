...
def simplify_docstring(docstring: str) -> str:
    ...
    for line in lines[1:]:
        ...
        if (
            in_args
            and stripped
            and not stripped.startswith(("Returns:", "Raises:", "Example:"))
        ):
            ...
            if ":" in stripped and not stripped.startswith(" "):
                ...
                if len(param_line) == 2:
                    # 只保留第一句话
                    desc = (
                        param_line[1].strip().split("。")[0] + "。"
                        if "。" in param_line[1]
                        else param_line[1].strip()
                    )
...
if __name__ == "__main__":
    process_file(
        "src/strategy/sync_api.py.backup", "src/strategy/sync_api_simplified.py"
    )
